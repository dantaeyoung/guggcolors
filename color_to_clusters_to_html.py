...
json_data = open('jsons/avgColorsScale.json').read()
...
if lab == True:
    clusters, groups = kmeans2( numpy.array(id_to_lab.values()), clustern)
    id_to_groups = dict(zip(id_to_lab.keys(), groups))
else:
    clusters, groups = kmeans2( numpy.array(id_to_rgb.values()), clustern)
    id_to_groups = dict(zip(id_to_rgb.keys(), groups))
...
with open('index_%s.html' % clustern, 'w') as f:
    f.write('<html>\n')
    f.write('<head>\n')
    f.write('  <title>the 5 colors 1,715 architecture firms think the helsinki guggenheim should look like</title>\n')
    f.write('<meta name="description" content="the %s colors 1,715 architecture firms think the helsinki guggenheim should look like" />' % clustern)
    ...
    f.write('</head>\n')
    f.write('<body>\n')
    f.write('<div class="title">the %s colors<br> 1,715 architecture firms think<br> the helsinki guggenheim<br> should look like</div>\n' % clustern)
    ...
    for g, ids in groups_to_ids.iteritems():
        if lab == True:
            ...
            f.write('<div class="cluster cluster-%s" data-cluster="%s">\n' % (str(g), lab_to_rgb(rgb_to_lab(clusters[g]), hex=True)))
            f.write('  <div class="cluster-color" style="background-color:%s"></div>\n' % lab_to_rgb(rgb_to_lab(clusters[g]), hex=True))
        ...
        for thisid in ids:
            ...
            f.write('      <div class="image"><img style="border-color:%s" src="%s"></div>\n' % (thishex, urlResized))
            f.write('    </div>\n')
